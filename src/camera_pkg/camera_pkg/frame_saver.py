...
class FrameSaver(Node):
    def __init__(self):
        ...
        self.gray_subscriber = self.create_subscription(Image,'gray_frame',self.get_gray_frame,1)
    ...
    def get_gray_frame(self,gray_msg):
        try:
            ...
            gray_frame = self.bridge.imgmsg_to_cv2(gray_msg,'mono8')
        except CvBridgeError as e1:
            self.get_logger().error("RGB frame CV Bridge failed: "+str(e1))
        if gray_frame is not None:
            gray_filename = "gray"+ str(curr_epoch_time) + ".jpg"
    ...
    def get_depth_frame(self,depth_msg):
        try:
            ...
            depth_frame = self.bridge.imgmsg_to_cv2(depth_msg,'mono8')
        ...
        if depth_frame is not None:
            depth_filename = "depth_"+str(curr_epoch_time)+".jpg"
